...
trait EventImplicits {
  ...
  def Event2Json(e: Event, baseURL: String): JObject = ("id", e.id) ~ ("title", e.title) ~ ("description", e.description) ~
    ("tags", JArray(e.tags.map(x => JString(x.trim)).toList)) ~
    ("start", (e.start.getTime() / 1000)) ~
    ("end", (e.end.getTime() / 1000)) ~ ("url", baseURL + URLEncoder.encode(e.id + e.start))
  def EList2Json(l: List[Event], baseURL: String) = JArray(l map { Event2Json(_, baseURL) })
}